...
class TestRepeaterHandler_WithoutParameters(AsyncHTTPTestCase):
    def get_app(self):
    ...
    def boilerplate(self, response, **kwargs):
        ...
        if kwargs.get('method') != 'HEAD':
            assert response.body.decode().find(f"> {kwargs.get('method')} /test/without/parameters.ext HTTP/1.1") != -1
    ...
    def test_HTTP_method_GET_without_parameters(self):
        response = self.fetch('/test/without/parameters.ext',
            method='GET',
            )
        boilerplate = self.boilerplate(response,
            code=200,
            method='GET',
            )
...
## https://www.tornadoweb.org/en/stable/testing.html
class TestRepeaterHandler_TransferEncodingchunked(AsyncHTTPTestCase):
    def get_app(self):
        return make_app(debug=True, autoreload=False)
...
## https://www.tornadoweb.org/en/stable/testing.html
class TestRepeaterHandler_WithContentParameter(AsyncHTTPTestCase):
    def get_app(self):
        return make_app(debug=True, autoreload=False)


    def test_HTTP_method_GET_with_content_no_value(self):
        response = self.fetch('/test/with.ext?content',
            method='GET',
            )
        assert response.code == 200
        assert response.body.decode().startswith('# Hello World!') is False
        assert response.body.decode().find(f"> GET /test/with.ext?content HTTP/1.1") != -1


    def test_HTTP_method_GET_with_content_and_value(self):
        response = self.fetch('/test/with.ext?content=1024',
            method='GET',
            )
        assert response.code == 200
    ...
    def test_HTTP_method_GET_with_content_encoding_identity(self):
        response = self.fetch('/test/with.ext?content=1024&encoding=identity',
            method='GET',
            )
        assert response.code == 200
    ...
    def test_HTTP_method_GET_with_content_encoding_gzip(self):
        response = self.fetch('/test/with.ext?content=1024&encoding=gzip',
            method='GET',
            )
        assert response.code == 200
        assert int(response.headers.get('Content-Length')) < 1024
    ...
    def test_HTTP_method_GET_with_content_and_fill(self):
        response = self.fetch('/test/with.ext?content=1024&fill=aaaa',
            method='GET',
            )
        assert response.code == 200
        assert len(response.body) == 1024
    ...
    def test_HTTP_method_GET_with_content_and_lipsum(self):
        response = self.fetch('/test/with.ext?content=1024&fill=lipsum',
            method='GET',
            )
        assert response.code == 200
        assert len(response.body) == 1024
    ...
    def test_HTTP_method_GET_with_content_and_ascii(self):
        response = self.fetch('/test/with.ext?content=1024&fill=ascii',
            method='GET',
            )
        assert response.code == 200
        assert len(response.body) == 1024
...
## https://www.tornadoweb.org/en/stable/testing.html
class TestRepeaterHandler_WithDebugParameter(AsyncHTTPTestCase):
    def get_app(self):
        return make_app(debug=True, autoreload=False)


    def test_HTTP_method_GET_with_debug(self):
        response = self.fetch('/test/with.ext?debug',
            method='GET',
            )
        assert response.code == 200
        assert response.body.decode().startswith('# DEBUG: request.arguments') is True
        assert response.body.decode().find(f"> GET /test/with.ext?debug HTTP/1.1") != -1


## https://www.tornadoweb.org/en/stable/testing.html
class TestRepeaterHandler_WithDelayParameter(AsyncHTTPTestCase):
    def get_app(self):
        return make_app(debug=True, autoreload=False)
    ...
    def test_HTTP_method_GET_with_delay(self):
        ...
        response = self.fetch(f"/test/with.ext?delay=1.25&header=X-Start-Time:{start_time}",
            method='GET',
            )
        assert response.code == 200
...
## https://www.tornadoweb.org/en/stable/testing.html
class TestRepeaterHandler_WithHeaderParameter(AsyncHTTPTestCase):
    def get_app(self):
        return make_app(debug=True, autoreload=False)


    def test_HTTP_method_GET_with_header_add(self):
        response = self.fetch('/test/with.ext?header=x-wr-test:Test%20add',
            method='GET',
            )
        assert response.code == 200
        assert response.headers.get('X-Wr-Test') == 'Test add'
        assert response.body.decode().find(f"< X-Wr-Test: Test add") != -1


    def test_HTTP_method_GET_with_header_modify(self):
        response = self.fetch('/test/with.ext?header=server:Test%20modify',
            method='GET',
            )
        assert response.code == 200
        assert response.headers.get('Server') == 'Test modify'
        assert response.body.decode().find(f"< Server: Test modify") != -1


    def test_HTTP_method_GET_with_header_remove(self):
        response = self.fetch('/test/with.ext?header=Content-Type:',
            method='GET',
            )
        assert response.code == 200
...
## https://www.tornadoweb.org/en/stable/testing.html
class TestRepeaterHandler_WithQuiteParameter(AsyncHTTPTestCase):
    def get_app(self):
        return make_app(debug=True, autoreload=False)


    def test_HTTP_method_GET_with_quite(self):
        response = self.fetch('/test/with.ext?quiet',
            method='GET',
            )
        assert response.code == 200
        assert response.body.decode().startswith(f"> GET /test/with.ext?quiet HTTP/1.1") is True
        assert response.body.decode().endswith('<\n') is True


## https://www.tornadoweb.org/en/stable/testing.html
class TestRepeaterHandler_WithSetParameter(AsyncHTTPTestCase):
    def get_app(self):
        return make_app(debug=True, autoreload=False)


    def test_HTTP_method_GET_with_set_address_match(self):
        response = self.fetch('/test/with.ext?set=delay:0.25,status:599,addr:8.7.6.5',
            method='GET',
    ...
    def test_HTTP_method_GET_with_set_header_match(self):
        response = self.fetch('/test/with.ext?set=delay:0.25,status:599,host:test',
            method='GET',
            headers={'Host': 'test'},
            )
        assert response.code == 599
...
## https://www.tornadoweb.org/en/stable/testing.html
class TestRepeaterHandler_WithStatusParameter(AsyncHTTPTestCase):
    def get_app(self):
        return make_app(debug=True, autoreload=False)


    def test_HTTP_method_GET_with_status(self):
        response = self.fetch('/test/with.ext?status=418',
            method='GET',
            )
    ...
    def test_HTTP_method_GET_with_status_and_reason(self):
        response = self.fetch('/test/with.ext?status=672&reason=Special+Weirdness',
            method='GET',
            )
